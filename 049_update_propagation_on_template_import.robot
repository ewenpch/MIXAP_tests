...
Create activity and share as first account
    [Documentation]    Account 1 creates an activity and generates a template share code for it.
...
Import activity as second account
    [Documentation]    Account 2 imports the activity shared by account 1 using the template share code.
...
Create activity and share as first account - Slow 3G
    [Documentation]    Account 1 creates an activity and generates a template share code for it, under throttled network conditions.
    Open Web Application with alias    compte1
    Set Network Speed
    Sign In    testaccount2@mail.com    password123
    Sleep    15s
...
Import activity as second account - Slow 3G
    [Documentation]    Account 2 imports the activity shared by account 1 using the template share code, under throttled network conditions.
    Open Web Application with alias    compte2
    Set Network Speed
    Sign In    test3@example.com    password123
    Import Activity    ${sharecode}
