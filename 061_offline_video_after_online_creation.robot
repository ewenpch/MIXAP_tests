...
*** Test Cases ***
Create activity with video while online
    [Documentation]    Sign up a fresh, randomly-generated account (so this run doesn't add to a shared account's ever-growing history) and create an augmented activity with a video overlay while online.
    ...
    Next button
    Sleep    2s
    Edit Activity Title    offline video activity ${run_suffix}
    Next button
    Sleep    2s
    Snap the background
    Sleep    2s
